  ...
  return (
    <div className="glass-card w-full max-w-md rounded-[32px] p-8">
      <div className="mb-8">
        ...
        <p className="mt-3 text-sm text-muted">Sign in to manage subscriptions, billing, and secure access.</p>
      </div>
      ...
      >
        ...
        <div>
          ...
          <input id="password" value={password} onChange={(event) => setPassword(event.target.value)} type="password" className="h-12 w-full rounded-2xl border border-white/10 bg-black/20 px-4 outline-none transition focus:border-primary" placeholder="••••••••" required />
        </div>
        {error ? <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-3 py-2 text-sm text-red-300">{error}</div> : null}
        <Button className="w-full" size="lg" type="submit">{loading ? "Signing in..." : "Sign in"}</Button>
      </form>
